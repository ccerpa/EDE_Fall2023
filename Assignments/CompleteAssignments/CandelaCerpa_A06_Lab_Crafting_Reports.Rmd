...
Value <- c("EPA Air Quality System (AQS)", "2018-2019",
           "EPAair_O3_PM25_NC1819_Processed.csv")
EAQ_metadata <- data.frame(Item, Value) #create dataframe with table values
knitr::kable(EAQ_metadata, caption = "EPA Air Quality", align = "rl")
```
...
```{r project.setup, echo=TRUE, message=FALSE, warning=FALSE, results = FALSE}
#Import libraries
...
\newpage
```{r data.summary, echo = FALSE, output = TRUE}
...
```{r Ozone18.boxplot, echo = FALSE, output = TRUE, warning = FALSE, fig.cap = "Monthly Ozone, 2018", fig.align='left', fig.dim = c(5, 4)}
O3_18_data <- filter(epa_data, Year == "2018")
O3_2018 <- ggplot(O3_18_data) +
...
```{r Ozone19.boxplot, echo = FALSE, output = TRUE, warning = FALSE, fig.cap = "Monthly Ozone, 2019", fig.align='left', fig.dim = c(5, 4)}
O3_19_data <- filter(epa_data, Year == "2019")
O3_2019 <- ggplot(O3_19_data) +
  geom_boxplot(aes(x = factor(Month), y= Ozone)) +
  scale_x_discrete(labels = month.abb) +
  labs (
...
## Git Repository
https://github.com/ccerpa/EDE_Fall2023
